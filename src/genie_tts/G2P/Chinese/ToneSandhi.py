...
if __name__ == '__main__':
    ts = ToneSandhi()
    test_cases = [
        [('很', 'd'), ('好', 'a')],  # hěn hǎo -> hén hǎo
        [('老', 'a'), ('虎', 'n')],  # lǎo hǔ -> láo hǔ
        [('一', 'm'), ('个', 'q')],  # yī gè -> yí gè
        [('一', 'm'), ('天', 'n')],  # yī tiān -> yì tiān
        [('看', 'v'), ('一', 'm'), ('看', 'v')],  # kàn yi kàn
        [('不', 'd'), ('是', 'v')],  # bú shì
        [('不', 'd'), ('好', 'a')],  # bù hǎo (注: 单独处理时可能根据pypinyin默认值)
        [('妈', 'n'), ('妈', 'n')],  # mā ma
        [('朋', 'n'), ('友', 'n')],  # péng you (注意：如果分词没把朋友分在一起，可能不会变轻声，视merge逻辑而定)
        [('花', 'n'), ('儿', 'n')],  # huār
        [('总', 'b'), ('理', 'n')],  # zóng lǐ
        [('展', 'v'), ('览', 'v'), ('馆', 'n')]  # zhǎn lǎn guǎn -> zhán lán guǎn
    ]

    print(f"{'原词 (分词状态)':<18} | {'合并后':<8} | {'变调前拼音':<20} | {'变调后拼音 (Result)'}")
    print("-" * 85)

    for segment in test_cases:
        # A. 预处理合并 (Merge)
        merged_seg = ts.pre_merge_for_modify(segment)
        full_original_pinyin = []
        full_modified_pinyin = []
        for _word, _pos in merged_seg:
            # 1. 获取声母 (Initials)
            ...
            # 2. 获取韵母 (Finals) - 这是 modified_tone 需要的输入
            ...
            # 3. 计算变调
            modified_finals = ts.modified_tone(_word, _pos, orig_finals)

            # 4. 拼接显示 (处理部分拼音库可能返回空声母的情况)
            # 原始拼音
            orig_word_pinyin = []
            for _i, _f in zip(initials, orig_finals):
                orig_word_pinyin.append(_i + _f)
            full_original_pinyin.extend(orig_word_pinyin)
            # 变调后拼音 (声母 + 变调后的韵母)
            mod_word_pinyin = []
            for _i, _f in zip(initials, modified_finals):
                mod_word_pinyin.append(_i + _f)
            full_modified_pinyin.extend(mod_word_pinyin)

        # 打印
        raw_text = "+".join([x[0] for x in segment])
        merged_text = "/".join([x[0] for x in merged_seg])
        orig_str = " ".join(full_original_pinyin)
        final_str = " ".join(full_modified_pinyin)
        print(f"{raw_text:<24} | {merged_text:<10} | {orig_str:<25} | {final_str}")
